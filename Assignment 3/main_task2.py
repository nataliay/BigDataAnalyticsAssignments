...
for i in range(num_iteration):
    ...
    xy_ypred = taxilinesCorrected.map(lambda x: (float(x[0]), float(x[1]), (m * float(x[0]) + b)))
    
    #Calculate cost (sum(y - (mx+b))^2)/n
    cost = (xy_ypred.map(lambda x: ((x[1] - x[2])**2)).reduce(add))/n

    #Calculate derivatives 
    m_deriv = (-2.0/n) * xy_ypred.map(lambda x: (x[0] * (x[1] - x[2]))).reduce(add)
    b_deriv = (-2.0/n) * xy_ypred.map(lambda x: (x[1] - x[2])).reduce(add)
    ...
    #Update m and b using lr and new derivatives 
    m = m - lr * m_deriv
    b = b - lr * b_deriv
