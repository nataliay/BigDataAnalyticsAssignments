...
# (docID,  [dictionaryPos1, dictionaryPos2, dictionaryPos3...]) pairs
# and converts the dictionary positions to a bag-of-words numpy array...
allDocsAsNumpyArraysTest = allDictionaryWordsInEachDocTest.map(lambda x: (x[0], 
                                                                  freqArray (x[1], len(x[1]))))
...
# Get the version of dfArray where the i^th entry is the inverse-document frequency for the
# i^th word in the corpus
# only divide by dfArrayTest if it wasn't 0 (it is zero when the dictionary has words that are not
# in any of the testing documents). Otherwise, instead of division, replace by 0
...
ytest_ypred = allDocsAsNumpyArraysTFidfTest.map(lambda x: (x[0], x[1], 1 if np.dot(r, x[2]) > 0 
                                                           else 0))
