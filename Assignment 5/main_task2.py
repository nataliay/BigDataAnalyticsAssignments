...
while (i >= 0):
    ...
    #Calculate cost ((lambda*sum(w^2))/2 + sum(max(0,1 - y(w * x))) / n)
    cost = ((lam * np.sum(w**2)) / 2) + xy.map(lambda x: max(0, (1 - (x[0] * np.dot(w, x[1])))))\
                                                .reduce(add)
    
    #Calculate derivatives (sum(lambda*w)/n if 1 - y(w*x) <= 0 else sum(lambda*w - y*x)/n)
    w_deriv = xy.map(lambda x: 0 if (1 - (x[0] * np.dot(w, x[1]))) <= 0 else (- c*x[0]*x[1]))\
               .reduce(add) 
