...
# Task 4 Print count of docs per cluster(category)
cluster_DocCounts = finalresult.map(lambda x: (x[0], 1)).reduceByKey(add)
print('The number of documents assigned to each cluster:\n', cluster_DocCounts.collect())

# Task 4 - print out the top three ”real” categories for all of the documents assigned to the cluster
cluster_catCounts = finalresult.map(lambda x: (x[0], re.findall("\/(.*?)\/", x[1])[0])).map(lambda x: ((x[0],x[1]),1)).reduceByKey(add)

# The percentage of documents that are assigned to the cluster that fall in that category
clusterCatPercentages = cluster_catCounts.map(lambda x: (x[0][0], (x[0][1], x[1]))).join(cluster_DocCounts)\
.map(lambda x: (x[0], np.divide(x[1][0][1], x[1][1])*100, x[1][0][0]))

topCatsArray = [] 
for i in range(20): 
    topCatsArray.append(clusterCatPercentages.filter(lambda x: x[0] == i).top(3, lambda x: x[1]))

print('Top 3 categories assigned to each cluster and percentage of documents that are assigned to the cluster that fall in that category:\n',
      topCatsArray)
